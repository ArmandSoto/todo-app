...
function App() {
  ...
  function toggleTodoList(event) {
    if (
      (event.target.innerHTML === "Inbox" && !showTodoList) ||
      (event.target.innerHTML === "Completed" && showTodoList)
    ) {
  ...
  return (
    <main className="text-base font-extralight tracking-wide h-screen ">
      ...
      <div className="flex">
        {sidebarIsOpen && (
          <nav className="border-2 w-auto">
            {" "}
            <Sidebar handleToggleTodoList={toggleTodoList} />{" "}
          </nav>
        )}
